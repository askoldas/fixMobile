...
import styles from './UserProfile.module.scss'; // You can move the original SCSS here
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        {['street', 'city', 'zip', 'country'].map((field) => (
          <div key={field}>
            ...
            <input type="text" value={form.legal[field]} onChange={(e) => handleChange(e, 'legal', field)} className={styles.input} />
          </div>
        ))}

        <div className={styles.checkboxRow}>
          <input type="checkbox" checked={form.sameAsLegal} onChange={handleSameAsLegalToggle} />
        ...
        {!form.sameAsLegal && (
          <>
            ...
            {['street', 'city', 'zip', 'country'].map((field) => (
              <div key={field}>
                <label>{field.charAt(0).toUpperCase() + field.slice(1)}</label>
                <input type="text" value={form.delivery[field]} onChange={(e) => handleChange(e, 'delivery', field)} className={styles.input} />
              </div>
            ))}
          </>
        )}
